...
import { charges } from "@/utils/breadcrumbs";
import DropdownChargesReadOnly from "@/components/Dropdown/DropdownChargesReadOnly";
...
const Page = () => {
  ...
  const fetchData = async (cursor: string | null = null, isNext = true) => {
    setLoading(true);
    if (!rera) return;
    const response = await getAllPreferenceLocationCharges(
      rera,
      cursor || undefined
    );
  ...
  return (
    <div className={`container ${styles.container}`}>
      ...
      <CustomBreadcrumbs items={charges} />
      {loading ? (
