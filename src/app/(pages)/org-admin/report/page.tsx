...
const Page = () => {
	...
	const handleTowerChange = async (e: SelectChangeEvent<string>) => {
		...
		// creating flats map
		const flatsMap = new Map<string, FlatInfo>();
		report.data.flats.forEach((flat: FlatInfo) => {
			flatsMap.set(flat.id, flat);
		});

		// attaching flat details to sold and unsold flats

		// const flatsMap: Record<string, FlatInfo> = {};
		// report.data.flats.forEach((flat: FlatInfo) => {
		...
		const enrichedBreakdown = report.data.paymentBreakdown.map(
			(breakdown: {
				paidItems: { flatId: string }[] | null;
				unpaidItems: { flatId: string }[] | null;
			}) => {
				const attachFlatInfo = (items: { flatId: string }[] | null) => {
					if (!items) return null;
					return items.map((item) => ({
						...item,
						flatInfo: flatsMap.get(item.flatId) || null,
					}));
				};
																	...
																	>
																		...
																		{item
																			...
																			<>
																				...
																				>
																					{`${item.flatInfo.saleDetail.companyCustomer.name} (${item.flatInfo.saleDetail.companyCustomer.companyPan})`}
																				</span>
																			</>
																		) : (
																			<>
