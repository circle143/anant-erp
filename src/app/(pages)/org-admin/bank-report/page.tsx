...
import { getAllSocietyBanks, getSocieties, getBankReport } from "@/redux/action/org-admin";
...
const Page = () => {
  ...
  const handleClearFilter = async () => {
    ...
    if (formik.values.society && formik.values.bank) {
      setLoading(true);
      const data = await getBankReport(formik.values.society, formik.values.bank);
      setReport(data);
      setLoading(false);
    }
  };
        ...
        <div className={styles.dateGroup}>
          ...
          <div className={styles.buttonGroup}>
            ...
            <button
              ...
              disabled={loading || !formik.values.society || !formik.values.bank}
            >
              Clear Filter
            </button>
          </div>
      ...
      {report && (
        <div className={styles.reportContainer}>
          ...
          <div className={styles.summarySection}>
            <h3>Total Amount: {formatIndianCurrencyWithDecimals(report.data.totalAmount)}</h3>
          </div>
          <div className={styles.bankDetails}>
            <h3>Bank Details</h3>
            <div className={styles.detailGrid}>
              <p><strong>Bank Name:</strong> {report.data.Details.name}</p>
              <p><strong>Account Number:</strong> {report.data.Details.accountNumber}</p>
              <p><strong>Registered On:</strong> {formatDate(report.data.Details.createdAt)}</p>
              <p><strong>Last Updated:</strong> {formatDate(report.data.Details.updatedAt)}</p>
            </div>
          </div>
          ...
          {report.data.Details.clearedReceipts?.length > 0 ? (
            <div className={styles.receiptsSection}>
              ...
              <table className={styles.detailsTable}>
                ...
                <tbody>
                  {report.data.Details.clearedReceipts.map((clearedReceipt: ClearedReceipt) => (
                    <tr key={clearedReceipt.receiptId}>
                      <td>{formatDate(clearedReceipt.receipt.dateIssued)}</td>
                      <td>{formatIndianCurrencyWithDecimals(clearedReceipt.receipt.amount)}</td>
                      <td>{clearedReceipt.receipt.mode}</td>
                      <td>{clearedReceipt.receipt.transactionNumber || "N/A"}</td>
                      <td>
                        {clearedReceipt.receipt.sale?.flat ? (
                          <>
                            {clearedReceipt.receipt.sale.flat.name} (Floor: {clearedReceipt.receipt.sale.flat.floorNumber}, 
                            Facing: {clearedReceipt.receipt.sale.flat.facing})
                          </>
                        ) : "N/A"}
                      </td>
                    <td>
  {clearedReceipt.receipt.sale?.companyCustomer ? (
    <>
      {clearedReceipt.receipt.sale.companyCustomer.name}
      {" "}
      (PAN: {clearedReceipt.receipt.sale.companyCustomer.companyPan})
    </>
  ) : clearedReceipt.receipt.sale?.owners?.length ? (
    <>
      {clearedReceipt.receipt.sale.owners.map((owner, index) => (
        <div key={owner.id || index}>
          {owner.firstName} {owner.lastName}
          {owner.panNumber && <> (PAN: {owner.panNumber})</>}
          {owner.aadharNumber && <> (Aadhar Number: {owner.aadharNumber})</>}
        </div>
      ))}
    </>
  ) : (
    "N/A"
                      ...
                      <td>{formatIndianCurrencyWithDecimals(clearedReceipt.receipt.cgst)}</td>
                      <td>{formatIndianCurrencyWithDecimals(clearedReceipt.receipt.sgst)}</td>
                      <td>{formatDate(clearedReceipt.createdAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <p className={styles.noData}>No cleared receipts found for this bank.</p>
          )}
        </div>
      )}
    </div>
