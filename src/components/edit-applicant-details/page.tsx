  ...
  const validationSchema = Yup.object()
    .shape({
      ...
      phoneNumber: Yup.string()
        .matches(
          /^(\+91)?[6-9]\d{9}$/,
          "Phone must be a valid 10-digit Indian number, with or without +91"
        )
      ...
      try {
        ...
        const formattedValues: formattedValues = {
          ...
          anniversaryDate: values.anniversaryDate
            ? new Date(values.anniversaryDate).toISOString().slice(0, 10)
            : "",
    ...
    return (
      <Box textAlign="center" mt={5}>
        <div className={styles.errorContainer}>
          <h3>No applicant data found for ID: {id}</h3>
          <p>
            The applicant you are trying to edit does not exist or is no longer
            available.
          </p>
          <Button
            variant="contained"
